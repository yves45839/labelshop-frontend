    ...
    <div className="bg-[var(--lr-steel-50)] min-h-screen">
        ...
        <div className="lr-container py-12 flex items-end justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="mt-2 text-white/60 text-sm max-w-xl">
              Sécurité électronique, vidéosurveillance, contrôle d'accès — par <span className="font-semibold text-white/90">Yves Roland OUIYA</span>
            </p>
          </div>
          {isAdmin && (
      ...
      <main className="lr-container py-10">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="lr-mono text-xs text-[var(--lr-steel-500)] hover:text-[var(--lr-navy-900)]">
              ✕ Effacer
            </button>
          )}
          ...
          <div className="grid gap-px bg-[var(--lr-border)] border border-[var(--lr-border)]">
            {filtered.map((blog, idx) => {
              ...
              return (
                <article
                  key={blog.id}
                  className="bg-white p-5 flex flex-col sm:flex-row justify-between gap-4 hover:bg-[var(--lr-steel-50)] transition-colors group"
                >
                  {/* Gauche */}
